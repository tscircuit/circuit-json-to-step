...
test(
  ...
  async () => {
    ...
    const solids = Array.from(repository.entries())
      .map(([, entity]) => entity)
      .filter((entity): entity is ManifoldSolidBrep => entity instanceof ManifoldSolidBrep)
    const boardSolids = solids.filter((entity) => entity.name === "KiCadStepMerge")
    expect(boardSolids.length).toBe(1)
    const componentSolids = solids.length - boardSolids.length
  expect(componentSolids).toBeGreaterThanOrEqual(EXPECTED_COMPONENT_CENTERS.length)
    ...
    try {
      ...
      if (boardBounds) {
        expect(boardBounds.max.x - boardBounds.min.x).toBeCloseTo(40, 1)
        expect(boardBounds.max.y - boardBounds.min.y).toBeCloseTo(20, 1)
      ...
      expect(usedIndices.size).toBeGreaterThanOrEqual(EXPECTED_COMPONENT_CENTERS.length)
    } catch (error) {
