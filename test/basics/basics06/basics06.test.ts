...
test("basics06: skip mesh generation when all components have model_step_url", async () => {
  // Load STEP files from local fixtures
  const fsMap = await loadStepFilesFromCircuitJson(circuitJson)

  // This should NOT throw "Failed to generate component meshes" error
  // because all cad_components have model_step_url and mesh generation should be skipped
  const stepText = await circuitJsonToStep(circuitJson as any, {
    includeComponents: true,
    includeExternalMeshes: true,
    productName: "TestPCB_SkipMeshGen",
    fsMap,
  })
  ...
  expect(stepText).toContain("TestPCB_SkipMeshGen")
  ...
  const outputPath = "debug-output/basics06.step"
  await Bun.write(outputPath, stepText)

  console.log("✓ STEP file with external models generated successfully")
  console.log(`  - Solids created: ${solidCount}`)
  console.log(`  - STEP text length: ${stepText.length} bytes`)
  console.log(`  - Output: ${outputPath}`)
  console.log("✓ Mesh generation was correctly skipped (no components needed fallback)")
